...
func (s *SellerRepositoryImpl) FindAll(ctx context.Context, db *sql.Tx) []domain.Seller {
	SQL := "SELECT id, name, category, description, created_at FROM marketplace.sellers"
...
func (s *SellerRepositoryImpl) FindByID(ctx context.Context, db *sql.Tx, id int64) (domain.Seller, error) {
	SQL := "SELECT id, name, category, description, created_at FROM marketplace.sellers WHERE id=$1"
	...
	seller := domain.Seller{}
	if result.Next() {
		err := result.Scan(&seller.Id, &seller.Name, &seller.Category, &seller.Description, &seller.CreatedAt)
		helper.PanicIfError(err)
		return seller, nil
	} else {
...
func (s *SellerRepositoryImpl) Create(ctx context.Context, db *sql.Tx, seller domain.Seller) domain.Seller {
	SQL := "insert into marketplace.sellers (name, category, description) values ($1, $2, $3) returning id"
...
func (s *SellerRepositoryImpl) Update(ctx context.Context, db *sql.Tx, seller domain.Seller) domain.Seller {
	SQL := "update marketplace.sellers set name=$1, category=$2, description=$3 where id=$4"
...
func (s *SellerRepositoryImpl) Delete(ctx context.Context, db *sql.Tx, id int64) {
	SQL := "delete from marketplace.sellers where id=$1"
